...
function Ball({name, radius}: IBallProps) {
  const {height, squish} = useTheatre(
    name,
    ["height", "squish"],
    TLEasyAnimation
  )
  ...
  return (
    <group>
      ...
      >
        <planeGeometry
          attach="geometry"
          args={[width, height, width * 3, width * 3] as any}
        />
        <meshBasicMaterial
          attach="material"
          transparent
...
function Light({name, color, intensity}: ILightProps) {
  ...
  const [positionVector, setPositionVector] = useState<Vector3>(
    new Vector3(0, 0, 0)
  )
  ...
  return (
    <div className="bouncing-ball">
      <Canvas camera={camera}>
        <group rotation={new Euler(0.01, (scroll / scrollRate) * Math.PI, 0)}>
